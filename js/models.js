...
class StoryList {
  ...
  async addStory(user, { storyId, title, author, url, username, createdAt }) {
    const token = user.loginToken;
    const storyData = new Story({ storyId, title, author, url, username, createdAt });
    const response = await axios({
      method: "POST",
      url: `${BASE_URL}/stories`,
    ...
    storyData.storyId = response.data.story.storyId; 
    this.stories.unshift(storyData);
    user.ownStories.unshift(storyData);
...
class User {
  ...
  constructor(userData, token) {
    ...
    this.favorites = userData.favorites.map(story => new Story(story));
    this.ownStories = userData.ownStories.map(story => new Story(story));
